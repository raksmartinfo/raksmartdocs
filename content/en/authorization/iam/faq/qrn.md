...
云平台资源名称 (QRN) 被用来作为资源的唯一标识。当您需要在全局环境（比如 [IAM 策略](../../manual/policy)）中明确指定一项资源时，我们要求使用 QRN 。
...
- **平台分区(partition)**：目前作为保留字段。该字段是必填项，且固定为"petaexpress"。
...
> 4.`特殊情况`  
>
> 如 `qrn:petaexpress:iam::usr-xxxxxx:root` 用来表示该账号。
...
```txt
qrn:petaexpress:iam::usr-xxxxxx:root
```

资源 id 为`i-xxxxxx`的云服务器实例：

```txt
qrn:petaexpress:ecm:pek3:usr-xxxxxx:instance/i-xxxxxx
```

资源 id 为`iamp-xxxxxx`的策略：

```txt
qrn:petaexpress:iam::usr-xxxxxx:policy/iamp-xxxxxx
```

资源 id 为`iamr-xxxxxx`的身份：

```txt
qrn:petaexpress:iam::usr-xxxxxx:role/iamr-xxxxxx
```
