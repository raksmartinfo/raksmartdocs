...
keyword: PetaExpress, 青云, DNS, 子域名
weight: 6
draft: false
---
...
PetaExpress DNS 支持添加子域名，并独立管理子域名。

## 添加子域名

1. 登录 PetaExpress 控制台。
...
5. （可选）验证子域名所有权。
   ...
   如果主域名已托管在 PetaExpress 且使用相同的用户帐号来添加子域名，则无需验证即可直接添加。

6. 在域名添加窗口，单击**添加**，返回域名列表，子域名添加成功。

### 验证所有权

为了防止恶意测试，若**主域名尚未被 PetaExpress 当前用户帐号接管**，需验证添加的子域名的所有权。
...
2. 配置上级域名验证
   ...
   - 下图所示上级域名托管在 PetaExpress DNS 的不同用户帐号下，需要登录到该用户帐号，并设置验证。
...
当 PetaExpress 主域名解析记录中，存在该子域名解析记录时。在独立管理的子域名后，系统将自动将子域名解析记录（NS 类型除外）转移到子域名解析记录列表下，且不再允许在主域管理中添加该子域名解析记录。
...
PetaExpress 支持托管拆分的子域名。为进一步加强对子域名的管理，可将子域名托管到 PetaExpress 。
...
2. 查看并复制 PetaExpress DNS 服务器。
    ...
    如图，当前域名需要将 DNS 服务器修改为 `ns3.routewize.com` 和 `ns4.routewize.com` ，PetaExpress DNS 才会接管。

3. 登录主域名的解析商管理控制台，为该子域名添加 NS 类型解析记录，记录值填写已复制的服务器地址。

    ![添加NS解析记录](../_images/subzone_9.png)

4. 请耐心等待 PetaExpress 接管子域名，最长不超过72小时。
